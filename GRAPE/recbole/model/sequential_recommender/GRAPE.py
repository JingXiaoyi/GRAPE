...
class ACGMultiHeadAttention(nn.Module):
    ...
    def forward(self, input_tensor, attribute_table, position_embedding, hidden_state_attr, attention_mask, fusion_w, fusion_wc):
        ...
        # ac_attention = torch.permute(ac_raw_attention, (0, 2, 3, 1, 4)) jing
        # ac_attention_c = torch.permute(ac_raw_attention_c, (0, 2, 3, 1, 4)) jing
        ac_attention = ac_raw_attention.permute((0, 2, 3, 1, 4))
        ac_attention_c = ac_raw_attention_c.permute((0, 2, 3, 1, 4))
...
class GRAPE(SequentialRecommender):
    ...
    def position_embedding_jing(self, position_ids):
        max_len = 0
        for list in position_ids:
            max_len = max(max_len, len(list(filter(lambda x: x != 0, list))))
        pe = torch.zeros(max_len, self.hidden_size)
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, self.hidden_size, 2).float() * (-torch.log(torch.tensor(10000.0)) / d_model))
