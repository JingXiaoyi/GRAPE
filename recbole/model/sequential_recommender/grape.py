...
class GRAPE(SequentialRecommender):

    def __init__(self, config, dataset):
        super(GRAPE, self).__init__(config, dataset)
        ...
        #Jing
        self.attribute_hidden_size = [self.attribute_hidden_size[0] for _ in range(len(self.selected_features))]
        ...
        self.jing_green_alpha = 0.95
        self.jing_green_loss = 0
        self.order_jing = [[], [0, 1, 2], [0, 2, 1], [1, 0, 2], [1, 2, 0], [2, 0, 1], [2, 1, 0], ]
        self.jing_green_beta = [0.7, 0.7, 0.65]
        ...
        def norm_jing(l, revise):
            ...
            for i in range(len(l)):
                ...
                if revise:
                    l[i] = 1-l[i]
        norm_jing(e_all, True)
        norm_jing(n_all, False)
        norm_jing(m_all, False)
        ...
        self.jing_dict_green[self.jing_id_idx[0]] = [0, 0, 0]
    ...
    def calculate_loss(self, interaction):
        ...
        if self.loss_type == 'BPR':
            ...
            _, picked_item_jing = torch.topk(logits, 1)
            picked_item_jing = picked_item_jing.squeeze()

            green_jing = 0
            pad_num_jing = 0
            for i in range(len(picked_item_jing)):
                item_now = picked_item_jing[i].item()
                if item_now == 0:
                    pad_num_jing += 1
                green_now = self.jing_dict_green[self.jing_id_idx[item_now]]
                if self.jing_green_loss == 0:
                    green_jing += sum(green_now)
                else:
                    order_now = self.order_jing[self.jing_green_loss]
                    if green_now[order_now[0]] < self.jing_green_beta[order_now[0]]:
                        green_jing += green_now[order_now[0]]
                    elif green_now[order_now[1]] < self.jing_green_beta[order_now[1]]:
                        green_jing += green_now[order_now[1]]
                    elif green_now[order_now[2]] < self.jing_green_beta[order_now[2]]:
                        green_jing += green_now[order_now[2]]


            green_jing /= (len(picked_item_jing)-pad_num_jing)
...
#                    print('total_loss:{}\titem_loss:{}\tattribute_{}_loss:{}'.format(total_loss, loss, self.selected_features[0], attribute_loss))
                ...
                total_loss = loss
                total_loss = self.jing_green_alpha*total_loss + (1-self.jing_green_alpha)*green_jing
